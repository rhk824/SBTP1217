...
namespace SBTP.BLL
{
    ...
    public class XGYC_ZRJ_BLL
    {
        ...
        public static XGYC_ZRJ_BLL getBLL(string jh)
        {
            ...
            //注入液粘度：读取RSL3.DAT中**TPDSSJ 调前注入液粘度
            bll.ZRYND = string.IsNullOrEmpty(DatHelper.SaveTQZRYND()) ? 0 : double.Parse(DatHelper.SaveTQZRYND());
            //bll.ZRYND = 10;
            ...
            //调剖半径：读取RSL3.DAT中*STCS 调剖半径
            bll.TPBJ = DatHelper.ReadSTCS().Find(x => x.JH.Equals(jh)) == null ? 0 : double.Parse(DatHelper.ReadSTCS().Find(x => x.JH.Equals(jh)).YHBJ);
